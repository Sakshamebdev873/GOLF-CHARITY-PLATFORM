      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-dark-400 mb-8">
            Enter your latest Stableford scores (1–45). Your 5 most recent
            scores are used as your draw entry numbers.
          </p>
        </motion.div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-dark-300 leading-relaxed">
              ...
              <p>
                ...
                → 3. Your 5 scores become your entry numbers → 4. If they match
                the winning numbers,{" "}
                <span className="text-accent-400 font-medium">you win!</span>
              </p>
            </div>
          </div>
        </motion.div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <Ticket
                          className={`w-5 h-5 ${isEntered ? "text-brand-400" : "text-accent-400"}`}
                        />
                      </div>
                      <div>
                    ...
                    {isEntered ? (
                      <span className="flex items-center gap-2 px-4 py-2 bg-brand-500/10 text-brand-400 text-sm font-medium rounded-xl border border-brand-500/20">
                        <CheckCircle className="w-4 h-4" /> Entered with scores:{" "}
                        {scores.map((s: any) => s.score).join(", ")}
                      </span>
                    ) : !hasEnoughScores ? (
                      <span className="text-amber-400 text-sm">
                        Need {5 - scores.length} more score
                        {5 - scores.length > 1 ? "s" : ""} to enter ↓
                      </span>
                    ) : (
                      <button
        ...
        >
          ...
          {scores.length >= 5 && (
            <p className="text-amber-400/80 text-xs mt-3 flex items-center gap-1.5">
              ...
              You already have 5 scores. Adding a new one will replace your
              oldest score.
            </p>
          )}
        </motion.div>
        ...
        >
          ...
          {hasEnoughScores && (
            <p className="text-center text-dark-400 text-xs mb-6">
              ...
              — will be compared against the winning numbers in each draw you
              enter.
            </p>
          )}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-dark-500 text-sm">
                Add your 5 latest golf scores above to start entering draws.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {scores.map((score: any, i: number) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    {i === 0 && (
                      <span className="text-xs px-2 py-0.5 rounded bg-brand-500/10 text-brand-400">
                        Latest
                      </span>
                    )}
                  </div>
          ...
          {scores.length > 0 && scores.length < 5 && (
            <div className="mt-4 p-3 rounded-xl bg-amber-500/5 border border-amber-500/15 text-center">
              <p className="text-amber-400 text-sm font-medium">
                {5 - scores.length} more score{5 - scores.length > 1 ? "s" : ""}{" "}
                needed
              </p>
              <p className="text-dark-500 text-xs mt-1">
                You need exactly 5 scores to enter draws. Add{" "}
                {5 - scores.length} more above.
              </p>
            </div>
          )}
        </motion.div>
          ...
          >
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-display font-semibold text-white">
                    {unentered.length} open draw
                    {unentered.length > 1 ? "s" : ""} waiting for you!
                  </p>
                  <p className="text-dark-400 text-sm">
                    Your scores [{scores.map((s: any) => s.score).join(", ")}]
                    are ready. Go enter now.
                  </p>
                </div>
              </div>
